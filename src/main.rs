...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::Lighting { action } => match action {
            ...
            LightingAction::Ring { enable }       => api.lighting_ring(enable)?,
        },

        Command::Playback { action } => match action {
            PlaybackAction::Direct { enable }      => api.playback_direct_mode(enable)?,
            PlaybackAction::SpdifDirect { enable } => api.playback_spdif_direct_mode(enable)?,
        ...
        Command::Mic { action } => match action {
            ...
            MicAction::NoiseReduction { enable, level } => {
                api.mic_noise_reduction(enable)?;
                if let Some(l) = level {
                    api.mic_noise_reduction_level(l)?;
                }
            }
            MicAction::Aec { enable }         => api.mic_aec(enable)?,
            MicAction::SmartVolume { enable } => api.mic_smart_volume(enable)?,
        ...
        Command::Sbx { action } => match action {
            ...
            SbxAction::DialogPlus { profile, enable, value } => {
                api.sbx_effect_toggle(profile, AudioFeature::DialogPlusToggle, enable)?;
                if let Some(v) = value {
